...
Runs the C++ simulation multiple times while varying a parameter (iq_ref)
and plots the T-n / I-T / P-T / eta-T characteristic curves.
"""
...
# ── Settings ──────────────────────────────────────────────────────────────────
# Path to the executable (project root = one level above tools/)
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
_ROOT_DIR   = os.path.dirname(_SCRIPT_DIR)
EXE_PATH = os.path.join(_ROOT_DIR, "BrushlessDCMotor.exe")

# Fixed motor parameters (must match sim_params.hpp)
KT   = 0.0533                             # Nm/A    (sim_params.hpp: kKt)
KE   = 0.0533                             # V/(rad/s)(sim_params.hpp: kKe)
R    = 0.1                                # Ohm     (sim_params.hpp: kR)
B    = 1.0e-2 / (2.0 * np.pi)            # Nm/(rad/s)(sim_params.hpp: kB)
V_DC = 48.0                               # assumed DC bus voltage [V] (rough estimate for efficiency)

# iq_ref values to sweep [A] (FOC q-axis current reference)
IQ_REF_LIST = [20.0, 40.0, 60.0, 85.0]

# Number of T_load points per iq_ref
N_TLOAD_POINTS = 12

# Simulation span [s] (enough time to reach steady state)
SIM_SPAN = 5.0
...
def sweep_iq(iq_ref: float, color: str) -> dict:
    ...
    # Efficiency (rough estimate: Pout / (V_dc * I_rms) x 100)
    pin = V_DC * I
