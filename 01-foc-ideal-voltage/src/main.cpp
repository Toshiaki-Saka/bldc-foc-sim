...
int main(int argc, char* argv[])
{
    ...
    {
        if      (std::strcmp(argv[i], "--kt")         == 0 && i + 1 < argc) kt         = std::atof(argv[++i]);
        else if (std::strcmp(argv[i], "--ke")         == 0 && i + 1 < argc) ke         = std::atof(argv[++i]);
        else if (std::strcmp(argv[i], "--r")          == 0 && i + 1 < argc) resistance = std::atof(argv[++i]);
        else if (std::strcmp(argv[i], "--l")          == 0 && i + 1 < argc) inductance = std::atof(argv[++i]);
        else if (std::strcmp(argv[i], "--b")          == 0 && i + 1 < argc) viscous    = std::atof(argv[++i]);
        else if (std::strcmp(argv[i], "--j")          == 0 && i + 1 < argc) inertia    = std::atof(argv[++i]);
        else if (std::strcmp(argv[i], "--pole_pairs") == 0 && i + 1 < argc) pole_pairs = std::atof(argv[++i]);
        else if (std::strcmp(argv[i], "--wn")         == 0 && i + 1 < argc) wn         = std::atof(argv[++i]);
        else if (std::strcmp(argv[i], "--zeta")       == 0 && i + 1 < argc) zeta       = std::atof(argv[++i]);
        else if (std::strcmp(argv[i], "--iq_ref")     == 0 && i + 1 < argc) iq_ref     = std::atof(argv[++i]);
        else if (std::strcmp(argv[i], "--tload")      == 0 && i + 1 < argc) tload      = std::atof(argv[++i]);
        else if (std::strcmp(argv[i], "--span")       == 0 && i + 1 < argc) span       = std::atof(argv[++i]);
        ...
        else if (std::strcmp(argv[i], "--no-midpoint")   == 0) midpoint   = false;
        else if (std::strcmp(argv[i], "--decoupling")    == 0) decoupling = true;
        else if (std::strcmp(argv[i], "--no-decoupling") == 0) decoupling = false;
    ...
    if (!quiet)
    {
        ...
        print_tn_characteristics(kt, ke, resistance, viscous, iq_ref, tload, last_state.angular_vel);
