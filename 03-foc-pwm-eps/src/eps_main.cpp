// =============================================================================
//  eps_main.cpp  —  EPS integrated simulation entry point
// -----------------------------------------------------------------------------
//  Project     : bldc-foc-sim / 03-foc-pwm-eps
//  In addition to the BLDC motor and FOC controller, couples the EPS mechanism
//  (column, torsion bar, reduction gear, rack) to reproduce the assist
//  behavior in response to driver steering.
//
...
int main(int argc, char* argv[]) {
    ...
    if (!quiet) {
        ...
        std::printf("  Configuration: BLDC motor (FOC) + reduction gear + pinion/rack + spring load\n");
        std::printf("\n  --- Parameters ---\n");
        ...
        std::printf("  Torque sensor value          : %.4f Nm\n", gearbox_state.torsion_torque);
        std::printf("  Iq reference                 : %.2f A\n", last_iq_ref);
