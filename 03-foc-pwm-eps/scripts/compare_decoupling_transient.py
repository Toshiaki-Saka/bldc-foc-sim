...
dq axes. This term barely shows up in steady state (especially when id is approx 0),
so looking at steady state alone reveals almost no effect.

This script therefore uses the `--iq_step` option to step the q-axis current
reference partway through the simulation, deliberately forcing a transient. By
overlaying that transient interval for decoupling ON / OFF, it visualizes how the
following change:
