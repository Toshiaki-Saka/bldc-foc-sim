...
class VCurveCanvas(QWidget):
    """Widget that plots the V-curve of rack force vs. steering torque"""
    ...
    def refresh(self):
        ...
        self.ax.plot(-th, fr, color="#e74c3c", linewidth=1.5, linestyle="--", label="rack force (negative dir., mirrored)")
...
class MainWindow(QMainWindow):
    ...
    def _try_load_default(self):
        default = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), DEFAULT_CSV)
    ...
    def _save_png(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Select PNG save location", "eps_output.png", "PNG Files (*.png)")
        if not path:
            return
