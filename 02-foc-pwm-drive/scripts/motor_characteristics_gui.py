...
class MotorCharacteristicsApp:
    ...
    def _build_figure(self):
        ...
        self.fig.subplots_adjust(left=0.10, right=0.72, top=0.91, bottom=0.13)
        ...
        self.ax_eta.spines['right'].set_position(('axes', 1.36))
    ...
    def _update(self):
        ...
        # ── Re-apply facecolor (cla resets it) ──
        for ax in (self.ax_n, self.ax_i, self.ax_p, self.ax_eta):
            ax.set_facecolor(C['bg'])
        ...
        # ── Draw curves ──
        self.ax_n.plot(T,   N,   color=C['speed'],      lw=2.0, solid_capstyle='round')
        self.ax_i.plot(T,   I,   color=C['current'],    lw=2.0, solid_capstyle='round')
        self.ax_p.plot(T,   P,   color=C['power'],      lw=2.0, solid_capstyle='round')
        self.ax_eta.plot(T, eta, color=C['efficiency'], lw=2.0, solid_capstyle='round')

        # ── Mark max-power and max-efficiency operating points ──
        idx_p   = int(np.argmax(P))
        idx_eta = int(np.argmax(eta))
        self.ax_p.plot(T[idx_p],  P[idx_p],   'o', color=C['power'],
                       ms=6, zorder=5, mew=1.5, mec='white')
        self.ax_eta.plot(T[idx_eta], eta[idx_eta], 'o', color=C['efficiency'],
                         ms=6, zorder=5, mew=1.5, mec='white')

        # ── Axis limits & labels ──
        self.ax_n.set_xlim(0, Ts)
        self.ax_n.set_ylim(bottom=0)
        self.ax_i.set_ylim(bottom=0)
        ...
        self.ax_n.set_ylabel('SPEED  N  (r/min)',     color=C['speed'],      fontsize=9)
        self.ax_i.set_ylabel('CURRENT  I  (A)',       color=C['current'],    fontsize=9)
        self.ax_p.set_ylabel('OUTPUT  P  (W)',        color=C['power'],      fontsize=9)
        self.ax_eta.set_ylabel('EFFICIENCY  η  (%)', color=C['efficiency'], fontsize=9)
