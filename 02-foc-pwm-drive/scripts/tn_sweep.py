...
# ── Configuration ─────────────────────────────────────────────────────────────
# Path to the executable (assumed to be in the same directory as the script)
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
EXE_PATH = os.path.join(_SCRIPT_DIR, "..", "BrushlessDCMotor.exe")

# Fixed motor parameters (keep in sync with main.cpp)
KT   = 3.5 / 85.0                        # Nm/A
KE   = 3.5 / 85.0                        # V/(rad/s)
R    = 0.015                              # Ohm
B    = 1.0e-2 / (2.0 * np.pi)            # Nm/(rad/s)
V_DC = 400.0                              # assumed DC bus voltage [V] (approximation for efficiency calc)

# iq_ref values to sweep [A] (FOC q-axis current reference)
IQ_REF_LIST = [20.0, 40.0, 60.0, 85.0]

# How many T_load points to sample per iq_ref
N_TLOAD_POINTS = 12
...
def sweep_iq(iq_ref: float, color: str) -> dict:
    ...
    # Efficiency (approximation: Pout / (V_dc * I_rms) x 100)
    pin = V_DC * I
