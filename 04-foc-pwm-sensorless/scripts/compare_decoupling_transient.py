...
dq axes. Because the coupling term barely appears in steady state (especially when
id is approximately 0), its effect is almost invisible if you only look at the
steady state.

This script therefore uses the `--iq_step` option to step the q-axis current
reference partway through the simulation, deliberately forcing a transient.
By overlaying that transient region for decoupling ON / OFF, it visualizes how
the following change:

  - the settling speed and overshoot of the q-axis current iq
...
Note:
    This script assumes that the executable supports the `--iq_step <time> <iq>`
...
def find_executable() -> str:
    """Search for the pre-built executable."""
    candidates = [
        "./BrushlessDCMotor",
