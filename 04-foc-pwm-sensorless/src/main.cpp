...
//  controller, and runs the time-step simulation loop.
//  Results are written to a RESULT line (standard output) and to CSV files.
//
...
int main(int argc, char* argv[]) {
    ...
    // Motor / observer parameters (overridable via CLI)
    double kt         = kKt;
    double ke         = kKe;
    double r_ohm      = kR;
    double l_henry    = kL;
    ...
    double wn         = kWn;
    double zeta       = kZeta;
    double obs_lpf    = kObsLpfCutoff;
    double pll_kp_val = kPllKp;
    double pll_ki_val = kPllKi;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (std::strcmp(argv[i], "--obs_lpf") == 0 && i + 1 < argc)
            obs_lpf = std::atof(argv[++i]);
        else if (std::strcmp(argv[i], "--pll_kp") == 0 && i + 1 < argc)
            pll_kp_val = std::atof(argv[++i]);
        else if (std::strcmp(argv[i], "--pll_ki") == 0 && i + 1 < argc)
            pll_ki_val = std::atof(argv[++i]);
    }
    ...
    MotorModel model{MotorConfig{
        .inertia            = j_inertia,
        .coil_resistance    = r_ohm,
        .counter_emf        = ke,
        .torque_constant    = kt,
        .viscous_resistance = b_visc,
        .inductance         = l_henry,
    ...
    const double kKp = 2.0 * zeta * wn * l_henry - r_ohm;
    const double kKi = wn * wn * l_henry;
    ...
    for (int step = 0; step < kSteps; ++step) {
        ...
        if (step < kStartupSteps) {
            ...
            observer.force_sync(last_state.electrical_deg, last_state.angular_vel * pole_pairs);
            ...
            const double w =
                static_cast<double>(step - kStartupSteps) / static_cast<double>(kBlendSteps);
        ...
        omega_elec   = last_state.angular_vel * pole_pairs;
    }
    ...
    std::printf("RESULT omega_ss=%.6f iq_ss=%.6f id_ss=%.6f tload=%.6f te_ss=%.6f"
                ...
                last_v_rms, last_state.angle_error);
    ...
    if (!quiet) {
        ...
        print_tn_characteristics(kt, ke, r_ohm, b_visc, iq_ref, tload, last_state.angular_vel);
