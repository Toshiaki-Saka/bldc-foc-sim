...
MotorState MotorModel::update(const Eigen::Vector3d& input_voltage, double estimated_deg)
{
    if (!csv_ready_)
    {
        csv_.open(csv_path_);
        csv_ << "U,V,W,ElecDeg,Te,id,iq,omega,Tm,MechDeg,AngleError,DutyU,DutyV,DutyW,Vu,Vv,Vw,EstimatedDeg\n";
        csv_ready_ = true;
    }
    ...
    // True electrical angle from mechanical angle (no sensor correction — reference only)
    ...
    elec_deg_ = wrap360(mech_deg_ * pole_pairs_);

    // angle_error: sensorless estimation error (true − estimated) [deg]
    const double angle_error = wrap_diff(elec_deg_ - estimated_deg);
