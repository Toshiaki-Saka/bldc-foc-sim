...
def plot_vcurve(results: list[dict]):
    ...
    # ─ Iq command vs steering torque ─
    ax = axes[1, 0]
    ax.plot(th_full, iq_full, color="#2ecc71", lw=2.0,
            solid_capstyle="round", label="Iq command [A]", zorder=3)
    _style(ax, "assist map: Iq command vs steering torque",
           "steering torque Th [Nm]", "Iq command [A]")
    ax.legend(fontsize=9, facecolor=panel, edgecolor=spine, labelcolor=tc)
    ...
    out_path = os.path.join(_SCRIPT_DIR, "data", "eps_vcurve.png")
    fig.savefig(out_path, dpi=150, bbox_inches="tight", facecolor=bg)
    print(f"\nPNG saved: {out_path}")
    plt.show()


# ── Simple V-curve figure (eps_model.png right-side style) ─────────────────────
def plot_schematic_vcurve(results: list[dict]):
    """
    Generate and save a V-curve figure with arrow axes, a white background, and
    minimal labels, matching the right-hand side of eps_model.png.
    """
    matplotlib.rcParams["font.family"] = "Yu Gothic"
    matplotlib.rcParams["axes.unicode_minus"] = False
    ...
    # Build the V shape with both arms positive (left arm = mirror of right arm)
    th_full   = np.concatenate([-th[::-1], th])
    ...
    out_path = os.path.join(_SCRIPT_DIR, "data", "eps_vcurve_schematic.png")
...
def main():
    if not os.path.exists(EXE_PATH):
        print(f"[ERROR] Executable not found: {EXE_PATH}")
        print("  Build it first with CMake.")
        sys.exit(1)
    ...
    print(f"\nCollected {len(results)} points. Displaying graphs...")
    plot_vcurve(results)
