...
================================
Tool for comparing the effect of dq-axis decoupling control via the "transient response".

Decoupling control cancels the cross-coupling term (omega_e * L * i) between the
dq axes. Because that cross-coupling term barely appears in steady state
(particularly when id ≈ 0), its effect is nearly invisible if you only look at
the steady state.

This script therefore uses the `--iq_step` option to force a "transient" by
stepping the q-axis current command partway through the simulation. By overlaying
that transient region for decoupling ON / OFF, it visualizes how the following
change:

  - the settling speed / overshoot of the q-axis current iq
  - the leakage into the d-axis current id (cross-coupling = the axis-to-axis
    interference itself)
...
CLI arguments:
    ...
    --step_time  SEC   time at which the q-axis command steps [s] (default 0.5)
    --iq_before  A     q-axis current command before the step [A] (default 85)
    --iq_after   A     q-axis current command after the step [A]  (default 30)
    --tload      NM    load torque [Nm]                           (default 4.3)
    --out        PATH  output path for the figure
    --no-show          save the image only, without displaying a window
