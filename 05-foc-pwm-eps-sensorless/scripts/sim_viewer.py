...
CHART_GROUPS_MOTOR = [
    ...
    ("PWM duty cycle (three-phase)", [
        ("DutyU_pct", "U-phase duty cycle [%]", "#e74c3c"),
        ("DutyV_pct", "V-phase duty cycle [%]", "#2ecc71"),
        ("DutyW_pct", "W-phase duty cycle [%]", "#3498db"),
    ]),
...
class VCurveCanvas(QWidget):
    ...
    def refresh(self):
        ...
        self.ax.plot(-th, fr, color="#e74c3c", linewidth=1.5, linestyle="--",
                     label="Rack force (negative direction, mirrored)")
        self.ax.set_xlabel("Steering torque Th [Nm]")
        self.ax.set_ylabel("Rack force [N]")
        self.ax.set_title("V-curve: rack force vs steering torque")
...
class SettingsTab(QWidget):
    ...
    def _build_ui(self):
        ...
        for label, val in [
            ...
            ("B  — viscous friction:",    f"{self._B_DEFAULT:.8f}  Nm·s/rad"),
            ("J  — moment of inertia:",   "3.5e-4  kg·m²"),
            ("Number of pole pairs:",     "4  pair"),
        ]:
        ...
        mc_form.addRow("IqRef — q-axis current command:", self.iqref_sb)
        ...
        self.eps_dec_cb  = QCheckBox("dq-axis decoupling control")
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.tabs.addTab(self.pwm_widget, "PWM waveform (three-phase)")
        ...
        try:
            ...
            if os.path.exists(pwm_path):
                try:
                    ...
                    self.status.showMessage(
                        f"Load complete: {os.path.basename(path)}  (PWM: {len(df_pwm):,} rows)", 4000)
                except Exception as e_pwm:
                    self.status.showMessage(
                        f"Load complete: {os.path.basename(path)}  (PWM CSV error: {e_pwm})", 6000)
            else:
                self.status.showMessage(f"Load complete: {os.path.basename(path)}", 3000)
