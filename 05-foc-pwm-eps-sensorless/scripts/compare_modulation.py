...
=====================
Tool for comparing waveform differences between midpoint (min-max) modulation
ON/OFF and dq-axis decoupling control ON/OFF.

Runs the same simulation under the following 4 conditions and overlays the
iq / Te / omega / id waveforms so the effect of each feature can be inspected
visually.
...
CLI arguments:
    --exe   PATH   path to the executable (default: auto-detect the built BrushlessDCMotor)
    --span  SEC    simulation time [s]        (default 2.0)
    --iq_ref AMP   q-axis current command [A] (default 85)
    --tload  NM    load torque [Nm]           (default 4.3)
    --out   PATH   output path for the figure (default data/compare_modulation.png)
